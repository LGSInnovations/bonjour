      ...
      records
        ...
        .forEach(function (rr) {
          if (rr.type === 'SRV') {
            ...
            service.rawTxt = rr.data.map(function (data) {
              return Buffer.concat([Buffer.from([data.length]), data])
            }).reduce((a, c) => Buffer.concat([a, c]), Buffer.alloc(0))
            service.rawTxt = service.rawTxt.length > 0 ? service.rawTxt : Buffer.from([0])

            service.txt = rr.data.map(function (data) {
              return data.length > 1 ? txt.decode(Buffer.concat([Buffer.from([data.length]), data])) : txt.decode(data)
